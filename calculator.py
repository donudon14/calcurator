...
def arity(character: str) -> int:
    assert character in OPERATORS
    return OPERATORS[character][0]


def priority(character: str) -> int:
    assert character in OPERATORS or character in '()'
    return OPERATORS[character][1] if character in OPERATORS else -1


def operation(character: str):
    assert character in OPERATORS
...
def parse_identifier(expr: str, i: int) -> tuple:
    n, token = len(expr), []
...
def parse_number(expr: str, i: int) -> tuple:
...
assert str(Tree.from_infix('(0 + 1) * (2 - 3)')) == str_tree
